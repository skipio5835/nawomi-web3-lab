...
contract ArcRentalEscrow {
        ...
        external
    {
        ...
        if (msg.sender != rental.owner) revert Unauthorized();
        if (refundTo != rental.renter) revert InvalidAddress();
    ...
    function cancelRental(bytes32 rentalId, address payable refundTo, string calldata cancelURI) external {
        ...
        address expectedRefundTo = rental.renter == address(0) ? rental.owner : rental.renter;
        if (refundTo != expectedRefundTo) revert InvalidAddress();
