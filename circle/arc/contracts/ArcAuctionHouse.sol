...
contract ArcAuctionHouse {
    ...
    mapping(address bidder => uint256 amount) private _pendingReturns;
    ...
    event BidRefundWithdrawn(address indexed bidder, address indexed to, uint256 amount);
    event BidRefundCredited(address indexed bidder, uint256 amount);
    ...
    function bid(bytes32 auctionId) external payable {
        ...
        if (previousAmount > 0) {
            _pendingReturns[previousBidder] += previousAmount;
            emit BidRefundCredited(previousBidder, previousAmount);
        }

        emit BidPlaced(auctionId, msg.sender, msg.value, previousBidder, previousAmount);
    }

    function withdrawBidRefund(address payable to) external {
        ...
        emit BidRefundWithdrawn(msg.sender, to, amount);
    }
    ...
    function cancelAuction(bytes32 auctionId, string calldata cancelURI) external {
        ...
        if (refundAmount > 0) {
            _pendingReturns[refundedBidder] += refundAmount;
            emit BidRefundCredited(refundedBidder, refundAmount);
        }
