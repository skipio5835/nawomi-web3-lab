...
contract ArcAirdropCampaign {
    ...
    error CampaignActive(bytes32 campaignId, uint64 closesAt);
    ...
    error InvalidCampaignId(bytes32 expected, bytes32 actual);
    ...
    function createCampaign(
        ...
        _requireCampaignId(campaignId, label);
    ...
    function closeCampaign(bytes32 campaignId) external {
        ...
        if (block.timestamp <= campaign.closesAt) {
            revert CampaignActive(campaignId, campaign.closesAt);
        }

        uint256 refundAmount = campaign.totalFunded - campaign.totalClaimed;
        campaign.closed = true;

        emit CampaignClosed(campaignId, msg.sender, msg.sender, refundAmount);
        ...
        returns (uint256 allocation, bool claimed, uint256 claimable)
    {
        ...
        bool active = (
            campaign.owner != address(0)
                && !campaign.closed
                && block.timestamp <= campaign.closesAt
        );
        claimable = claimed || !active ? 0 : allocation;
    }
    ...
    function _requireCampaignId(bytes32 campaignId, string calldata label) private view {
        bytes32 expectedCampaignId = keccak256(abi.encodePacked(msg.sender, label));
        if (campaignId != expectedCampaignId) {
            revert InvalidCampaignId(expectedCampaignId, campaignId);
        }
    }
}
