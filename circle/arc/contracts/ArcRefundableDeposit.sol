...
contract ArcRefundableDeposit {
    uint64 public constant RESOLUTION_WINDOW = 7 days;
    ...
    error ResolutionWindowPassed(uint64 reclaimAt);
    ...
    function createDeposit(
        ...
        if (deadline <= block.timestamp || deadline > type(uint64).max - RESOLUTION_WINDOW) {
    ...
    function refundDeposit(bytes32 depositId, address payable refundTo, string calldata resolutionURI) external {
        ...
        if (msg.sender != deposit.beneficiary && msg.sender != deposit.payer) revert Unauthorized();
        uint64 reclaimAt = deposit.deadline + RESOLUTION_WINDOW;
        if (msg.sender == deposit.payer && block.timestamp < reclaimAt) {
            revert DeadlineNotReached(reclaimAt);
        }
        if (refundTo != deposit.payer) revert InvalidAddress();
    ...
    function forfeitDeposit(bytes32 depositId, address payable payoutTo, string calldata resolutionURI) external {
        Deposit storage deposit = _requireOpenDeposit(depositId);
        if (msg.sender != deposit.beneficiary) revert Unauthorized();
        if (block.timestamp < deposit.deadline) revert DeadlineNotReached(deposit.deadline);
        uint64 reclaimAt = deposit.deadline + RESOLUTION_WINDOW;
        if (block.timestamp >= reclaimAt) revert ResolutionWindowPassed(reclaimAt);
        if (payoutTo != deposit.beneficiary) revert InvalidAddress();
        if (bytes(resolutionURI).length == 0) revert InvalidText();
