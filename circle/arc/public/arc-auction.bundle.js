...
function formatAbiParameters(abiParameters) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    params2 += formatAbiParameter(abiParameter);
    if (i !== length - 1)
      params2 += ", ";
  }
  return params2;
...
function parseEventSignature(signature, structs = {}) {
  ...
  const params2 = splitParameters(match.parameters);
  const abiParameters = [];
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter(params2[i], {
...
function parseErrorSignature(signature, structs = {}) {
  ...
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter(params2[i], { structs, type: "error" }));
...
function parseConstructorSignature(signature, structs = {}) {
  ...
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter(params2[i], { structs, type: "constructor" }));
  return {
    type: "constructor",
    stateMutability: match.stateMutability ?? "nonpayable",
...
function parseAbiParameter(param, options) {
  ...
  if (isTuple) {
    type = "tuple";
    const params2 = splitParameters(match.type);
    const components_ = [];
    const length = params2.length;
    for (let i = 0; i < length; i++) {
      components_.push(parseAbiParameter(params2[i], { structs }));
    }
...
function splitParameters(params2, result = [], current = "", depth = 0) {
  const length = params2.trim().length;
  for (let i = 0; i < length; i++) {
    const char = params2[i];
    const tail = params2.slice(i + 1);
    switch (char) {
      case ",":
...
function formatAbiParams(params2, { includeName = false } = {}) {
  if (!params2)
    return "";
  return params2.map((param) => formatAbiParam(param, { includeName })).join(includeName ? ", " : ",");
}
...
var init_abi = __esm({
  "node_modules/viem/_esm/errors/abi.js"() {
    ...
    AbiDecodingDataSizeTooSmallError = class extends BaseError2 {
      constructor({ data, params: params2, size: size5 }) {
        super([`Data size of ${size5} bytes is too small for given parameters.`].join("\n"), {
          metaMessages: [
            `Params: (${formatAbiParams(params2, { includeName: true })})`,
    ...
    DecodeLogDataMismatch = class extends BaseError2 {
      constructor({ abiItem, data, params: params2, size: size5 }) {
        super([
...
function decodeAbiParameters(params2, data) {
  const bytes = typeof data === "string" ? hexToBytes(data) : data;
  const cursor = createCursor(bytes);
  if (size(bytes) === 0 && params2.length > 0)
...
function getAction(client, actionFn, name) {
  ...
  return (params2) => actionFn(client, params2);
}
...
function getHttpRpcClient(url_, options = {}) {
  ...
  return {
    async request(params2) {
      const { body, fetchFn = options.fetchFn ?? fetch, onRequest = options.onRequest, onResponse = options.onResponse, timeout = options.timeout ?? 1e4 } = params2;
      const fetchOptions = {
        ...options.fetchOptions ?? {},
        ...params2.fetchOptions ?? {}
      };
...
function formatAbiParameters2(abiParameters) {
  ...
  for (let i = 0; i < length; i++) {
    const abiParameter = abiParameters[i];
    params2 += formatAbiParameter2(abiParameter);
    if (i !== length - 1)
      params2 += ", ";
  }
  return params2;
...
var InvalidAbiParametersError2 = class extends BaseError4 {
  constructor({ params: params2 }) {
    super("Failed to parse ABI parameters.", {
      details: `parseAbiParameters(${JSON.stringify(params2, null, 2)})`,
...
function parseEventSignature2(signature, structs = {}) {
  ...
  const params2 = splitParameters2(match.parameters);
  const abiParameters = [];
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter3(params2[i], {
...
function parseErrorSignature2(signature, structs = {}) {
  ...
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter3(params2[i], { structs, type: "error" }));
  return { name: match.name, type: "error", inputs: abiParameters };
}
function parseConstructorSignature2(signature, structs = {}) {
  ...
  const length = params2.length;
  for (let i = 0; i < length; i++)
    abiParameters.push(parseAbiParameter3(params2[i], { structs, type: "constructor" }));
  return {
    type: "constructor",
    stateMutability: match.stateMutability ?? "nonpayable",
    inputs: abiParameters
...
function parseAbiParameter3(param, options) {
  ...
  if (isTuple) {
    type = "tuple";
    const params2 = splitParameters2(match.type);
    const components_ = [];
    const length = params2.length;
    for (let i = 0; i < length; i++) {
      components_.push(parseAbiParameter3(params2[i], { structs }));
    }
    components = { components: components_ };
  } else if (match.type in structs) {
    type = "tuple";
...
function splitParameters2(params2, result = [], current = "", depth = 0) {
  const length = params2.trim().length;
  for (let i = 0; i < length; i++) {
    const char = params2[i];
    const tail = params2.slice(i + 1);
...
init_browser_buffer_global();
function parseAbiParameters2(params2) {
  const abiParameters = [];
  if (typeof params2 === "string") {
    const parameters = splitParameters2(params2);
    const length = parameters.length;
    for (let i = 0; i < length; i++) {
      abiParameters.push(parseAbiParameter3(parameters[i], { modifiers: modifiers2 }));
    }
  } else {
    const structs = parseStructs2(params2);
    const length = params2.length;
    for (let i = 0; i < length; i++) {
      const signature = params2[i];
      if (isStructSignature2(signature))
        continue;
  ...
  if (abiParameters.length === 0)
    throw new InvalidAbiParametersError2({ params: params2 });
...
function weierstrassPoints2(opts) {
  ...
  class Point2 {
     ...
     */
    static fromHex(hex) {
      const P = Point2.fromAffine(fromBytes5(ensureBytes2("pointHex", hex)));
      P.assertValidity();
      return P;
    }
    // Multiplies generator point by privateKey.
...
init_browser_buffer_global();
async function watchAsset(client, params2) {
...
function http(url, config = {}) {
  ...
  return ({ chain, retryCount: retryCount_, timeout: timeout_ }) => {
    ...
    return createTransport({
      ...
      methods,
      name,
      async request({ method, params: params2 }, options) {
        const body = { method, params: params2 };
...
var CONTRACT_KEY = "ArcAuctionHouse.contractAddress";
var DEFAULT_ACCOUNT = "0x0000000000000000000000000000000000000000";
...
var account = null;
var contractAddress = localStorage.getItem(CONTRACT_KEY) ?? "";
var currentAuction = null;
var pendingRefund = 0n;
var el = {
  auctionId: document.querySelector("#auctionId"),
  auctionStatus: document.querySelector("#auctionStatus"),
  bidCount: document.querySelector("#bidCount"),
  cancelAuction: document.querySelector("#cancelAuction"),
  cancelURI: document.querySelector("#cancelURI"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createAuction: document.querySelector("#createAuction"),
  deployContract: document.querySelector("#deployContract"),
  durationMinutes: document.querySelector("#durationMinutes"),
  highestBid: document.querySelector("#highestBid"),
  highestBidder: document.querySelector("#highestBidder"),
  metadataURI: document.querySelector("#metadataURI"),
  minBid: document.querySelector("#minBid"),
  nativeBalance: document.querySelector("#nativeBalance"),
  pendingRefund: document.querySelector("#pendingRefund"),
  placeBid: document.querySelector("#placeBid"),
  raiseBid: document.querySelector("#raiseBid"),
  raiseBidButton: document.querySelector("#raiseBidButton"),
  refresh: document.querySelector("#refresh"),
  refundTo: document.querySelector("#refundTo"),
  saveContract: document.querySelector("#saveContract"),
  sellerAddress: document.querySelector("#sellerAddress"),
  settleAuction: document.querySelector("#settleAuction"),
  settlementTo: document.querySelector("#settlementTo"),
  settlementURI: document.querySelector("#settlementURI"),
  statusLine: document.querySelector("#statusLine"),
  title: document.querySelector("#title"),
  walletAddress: document.querySelector("#walletAddress"),
  withdrawRefund: document.querySelector("#withdrawRefund")
};
...
function applyParam(name, input) {
...
if (el.contractAddress.value && isAddress(el.contractAddress.value)) {
  contractAddress = el.contractAddress.value;
...
function setStatus(message, hash3) {
  el.statusLine.innerHTML = hash3 ? `${escapeHtml(message)} <a href="${txUrl(hash3)}" target="_blank" rel="noreferrer">${hash3}</a>` : escapeHtml(message);
}
...
function auctionFromRaw(value) {
  ...
  return {
    seller: value[0],
    settlementTo: value[1],
    highestBidder: value[2],
    minBid: value[3],
    highestBid: value[4],
    settledAmount: value[5],
    bidCount: value[6],
    createdAt: value[7],
    endsAt: value[8],
    ...
    title: value[11],
    metadataURI: value[12],
    settlementURI: value[13],
    cancelURI: value[14]
  };
}
function updateAuctionIdDisplay() {
  try {
...
function renderAuction() {
  ...
  el.auctionStatus.textContent = currentAuction.closed ? currentAuction.canceled ? "canceled" : "settled" : "open";
  ...
  el.highestBidder.innerHTML = currentAuction.highestBidder === ZERO_ADDRESS ? "-" : `<a href="${addressUrl(currentAuction.highestBidder)}" target="_blank" rel="noreferrer">${shortValue(
...
function renderPendingRefund() {
  el.pendingRefund.textContent = `${formatEther(pendingRefund)} USDC`;
  updateActions();
}
function canSettle() {
  ...
  return BigInt(Math.floor(Date.now() / 1e3)) >= currentAuction.endsAt;
}
function updateActions() {
  ...
  el.withdrawRefund.disabled = !hasWallet || !hasContract || pendingRefund === 0n;
}
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcAuctionHouse.json").then(
...
async function refreshPendingRefund() {
  if (!account || !contractAddress || !isAddress(contractAddress)) {
    pendingRefund = 0n;
    renderPendingRefund();
    return;
  }
  pendingRefund = await publicClient.readContract({
    address: contractAddress,
    abi: arcAuctionAbi,
...
function endsAtValue() {
  ...
  return BigInt(Math.floor(Date.now() / 1e3) + Math.round(minutes * 60));
}
async function createAuction() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [auctionId(), settlementTo, minBid, endsAtValue(), title, metadataURI],
      account,
      chain: arcTestnet
    });
...
async function bidAmount(amount, label) {
  if (!walletClient || !account) await connect();
  if (!walletClient || !account || !contractAddress) return;
  try {
    setStatus(`${label}...`);
...
async function withdrawRefund() {
  ...
  try {
    ...
    setStatus("Refund withdrawal submitted:", hash3);
    await waitForSuccess(hash3, "Withdraw bid refund");
    await refreshBalance();
    await refreshPendingRefund();
    setStatus("Bid refund withdrawn:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
el.withdrawRefund.addEventListener("click", () => void withdrawRefund());
