...
var STORAGE_KEY = "arcSubscription.records";
var CONTRACT_KEY = "arcSubscription.contractAddress";
var DAY_SECONDS = 86400;
var arcSubscriptionAbi = [
  {
    inputs: [
...
el.cycles.value = "1";
el.reference.value = `arc-subscription-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`;
...
function randomPlanId(reference) {
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcSubscription.json").then((response) => response.json());
...
function subscriptionFromRaw(value) {
  if (Array.isArray(value)) {
    return {
      subscriber: value[0],
      paidThrough: value[2],
  ...
  try {
    ...
    if (subscription.subscriber !== "0x0000000000000000000000000000000000000000") {
      ...
      if (subscription.paidThrough > 0n) {
        record.paidThrough = new Date(Number(subscription.paidThrough) * 1e3).toISOString();
      }
    }
...
async function createPlan() {
  ...
  try {
    ...
    setStatus(`Plan submitted: ${hash3}`);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    record.status = "plan-created";
    record.contractAddress = contractAddress;
    record.planTxHash = hash3;
    record.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    saveRecords();
    render();
    setStatus(`Plan created: ${hash3}`);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function subscribe() {
  ...
  try {
    ...
    setStatus(`Subscribe submitted: ${hash3}`);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    record.status = "subscribed";
    record.subscribeTxHash = hash3;
    record.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    saveRecords();
    await refreshBalance();
    await refreshSelected();
    setStatus(`Subscription paid: ${hash3}`);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function cancelSubscription() {
  ...
  try {
    ...
    record.cancelTxHash = hash3;
    record.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    saveRecords();
    render();
    setStatus(`Subscription cancelled: ${hash3}`);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
