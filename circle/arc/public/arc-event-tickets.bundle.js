...
// circle/arc/src/arc-event-tickets.ts
init_browser_buffer_global();
...
var currentTicketId = 0n;
var el = {
  buyTicket: document.querySelector("#buyTicket"),
  ...
  organizerAddress: document.querySelector("#organizerAddress"),
  refresh: document.querySelector("#refresh"),
  refundTicket: document.querySelector("#refundTicket"),
  refundTo: document.querySelector("#refundTo"),
  saveContract: document.querySelector("#saveContract"),
  settleEvent: document.querySelector("#settleEvent"),
...
function renderEvent() {
  ...
  el.ticketStatus.textContent = currentTicket ? currentTicket.refunded ? "refunded" : currentTicket.checkedIn ? "checked in" : "purchased" : "none";
  updateActions();
}
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcEventTickets.json").then(
...
async function createEvent() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [eventId(), treasury, ticketPrice, maxSupply, title, metadataURI],
      account,
      chain: arcTestnet
    });
...
async function buyTicket() {
  ...
  try {
    ...
    setStatus("Buy submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshEvent();
    setStatus("Ticket purchased:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function checkIn() {
  ...
  try {
    ...
    setStatus("Check-in submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshEvent();
    setStatus("Ticket checked in:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function refundTicket() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [eventId(), currentTicketId, refundTo],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshEvent();
    setStatus("Ticket refunded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
