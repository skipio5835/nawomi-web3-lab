...
var CONTRACT_KEY = "arcRefundableDeposit.contractAddress";
var DEFAULT_ACCOUNT = "0x0000000000000000000000000000000000000000";
var DEFAULT_BENEFICIARY = "0x78131700be4a8f2d16eeb0cba3498d2e717f2cd3";
...
var el = {
  ...
  beneficiaryAddress: document.querySelector("#beneficiaryAddress"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createDeposit: document.querySelector("#createDeposit"),
  deadlineHours: document.querySelector("#deadlineHours"),
  deadlineStatus: document.querySelector("#deadlineStatus"),
  depositId: document.querySelector("#depositId"),
  depositStatus: document.querySelector("#depositStatus"),
  deployContract: document.querySelector("#deployContract"),
  forfeitDeposit: document.querySelector("#forfeitDeposit"),
  forfeitTo: document.querySelector("#forfeitTo"),
  ...
  refundDeposit: document.querySelector("#refundDeposit"),
  ...
  valueStatus: document.querySelector("#valueStatus"),
...
function depositFromRaw(value) {
  ...
  return {
    ...
    depositRef: value[7],
    metadataURI: value[8],
    resolutionURI: value[9]
  };
}
function updateDepositIdDisplay() {
  try {
...
function renderDeposit() {
  ...
  el.deadlineStatus.textContent = new Date(Number(currentDeposit.deadline) * 1e3).toLocaleString();
  el.refundTo.value = currentDeposit.payer;
  el.forfeitTo.value = currentDeposit.beneficiary;
  updateActions();
}
function updateActions() {
  ...
  const isPayer = Boolean(account && currentDeposit && account.toLowerCase() === currentDeposit.payer.toLowerCase());
  const isBeneficiary = Boolean(
    account && currentDeposit && account.toLowerCase() === currentDeposit.beneficiary.toLowerCase()
  );
  const reclaimAt = currentDeposit ? currentDeposit.deadline + 7n * 24n * 60n * 60n : 0n;
  const canRefund = isBeneficiary || isPayer && now >= reclaimAt;
  const canForfeit = Boolean(currentDeposit && isBeneficiary && now >= currentDeposit.deadline && now < reclaimAt);
  el.createDeposit.disabled = !hasWallet || !hasContract || hasDeposit;
  el.deployContract.disabled = !hasWallet;
  el.forfeitDeposit.disabled = !hasWallet || !hasContract || !open || !canForfeit;
  el.refresh.disabled = !hasContract;
  el.refundDeposit.disabled = !hasWallet || !hasContract || !open || !canRefund;
}
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcRefundableDeposit.json").then(
...
async function createDeposit() {
  ...
  try {
    ...
    const deadlineHours = Number(el.deadlineHours.value.trim());
    if (!Number.isFinite(deadlineHours) || deadlineHours <= 0) {
      throw new Error("Deadline hours must be greater than zero.");
    }
    const deadline = BigInt(Math.floor(Date.now() / 1e3) + Math.round(deadlineHours * 60 * 60));
    setStatus("Creating deposit...");
    const hash3 = await walletClient.writeContract({
      address: contractAddress,
      abi: arcDepositAbi,
      functionName: "createDeposit",
      args: [depositId(), beneficiary, deadline, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Deposit created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function refundDeposit() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [depositId(), refundTo, resolutionURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshDeposit();
    setStatus("Deposit refunded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function forfeitDeposit() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [depositId(), payoutTo, resolutionURI],
      account,
      chain: arcTestnet
    });
    setStatus("Forfeit submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshDeposit();
    setStatus("Deposit forfeited:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
