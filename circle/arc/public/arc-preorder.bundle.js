...
// circle/arc/src/arc-preorder.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcPreorderStore.contractAddress";
...
var el = {
  ...
  fulfillPreorder: document.querySelector("#fulfillPreorder"),
  fulfillmentURI: document.querySelector("#fulfillmentURI"),
  maxSupply: document.querySelector("#maxSupply"),
  metadataURI: document.querySelector("#metadataURI"),
  nativeBalance: document.querySelector("#nativeBalance"),
  placePreorder: document.querySelector("#placePreorder"),
  preorderId: document.querySelector("#preorderId"),
  preorderStatus: document.querySelector("#preorderStatus"),
  price: document.querySelector("#price"),
  productId: document.querySelector("#productId"),
  productStatus: document.querySelector("#productStatus"),
  refresh: document.querySelector("#refresh"),
  refundPreorder: document.querySelector("#refundPreorder"),
  ...
  settleProduct: document.querySelector("#settleProduct"),
...
function preorderFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function renderProduct() {
  ...
  el.preorderStatus.textContent = currentPreorder ? currentPreorder.refunded ? "refunded" : currentPreorder.fulfilled ? "fulfilled" : "ordered" : "none";
  updateActions();
}
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcPreorderStore.json").then(
...
async function createProduct() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [productId(), treasury, price, maxSupply, title, metadataURI],
      account,
      chain: arcTestnet
    });
    ...
    setStatus("Product created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function placePreorder() {
  ...
  try {
    ...
    setStatus("Preorder submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshProduct();
    setStatus("Preorder placed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function fulfillPreorder() {
  if (!walletClient || !account) await connect();
  ...
  try {
    ...
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshProduct();
    setStatus("Preorder fulfilled:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function refundPreorder() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [productId(), currentPreorderId, refundTo],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshProduct();
    setStatus("Preorder refunded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function settleProduct() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshProduct();
    setStatus("Product settled:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
