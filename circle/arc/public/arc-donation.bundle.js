...
// circle/arc/src/arc-donation.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcDonationJar.contractAddress";
...
var donationCount = 0n;
var latestDonation = null;
var el = {
  ...
  donationAmount: document.querySelector("#donationAmount"),
  donationCount: document.querySelector("#donationCount"),
  goalAmount: document.querySelector("#goalAmount"),
  latestDonation: document.querySelector("#latestDonation"),
  ...
  withdrawTo: document.querySelector("#withdrawTo")
};
...
function campaignFromRaw(value) {
  ...
  return {
    ...
    treasury: value[1],
    goalAmount: value[2],
    totalDonated: value[3],
    withdrawnAmount: value[4],
    createdAt: value[5],
...
function donationFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function renderCampaign() {
  ...
  el.campaignStatus.textContent = currentCampaign.active ? `${formatEther(currentCampaign.totalDonated)} / ${formatEther(currentCampaign.goalAmount)} USDC` : "closed";
  el.donationCount.textContent = donationCount.toString();
  el.latestDonation.innerHTML = latestDonation ? `${formatEther(latestDonation.amount)} USDC from <a href="${addressUrl(
    latestDonation.donor
  )}" target="_blank" rel="noreferrer">${shortValue(latestDonation.donor)}</a>` : `${formatEther(available)} USDC available`;
  if (available > 0n) {
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcDonationJar.json").then((response) => response.json());
...
async function createCampaign() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [campaignId(), treasury, goalAmount, title, metadataURI],
      account,
      chain: arcTestnet
    });
...
async function donate() {
  ...
  try {
    ...
    setStatus("Donation submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshCampaign();
    setStatus("Donation sent:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function withdraw() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [campaignId(), withdrawTo, amount],
      account,
      chain: arcTestnet
    });
    setStatus("Withdraw submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshCampaign();
    setStatus("Donation withdrawn:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
