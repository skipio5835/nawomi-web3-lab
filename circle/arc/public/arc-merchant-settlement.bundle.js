...
async function connect() {
  ...
  el.merchant.value = account;
  setStatus("Merchant wallet connected.");
  await refresh();
}
async function refresh() {
  if (!account) throw new Error("Connect MetaMask first.");
  const [native, usdc, eurc] = await Promise.all([publicClient.getBalance({ address: account }), ...Object.values(tokens).map((token) => publicClient.readContract({ address: token.address, abi: abi2, functionName: "balanceOf", args: [account] }))]);
  el.native.textContent = `${formatEther(native)} USDC`;
  el.usdc.textContent = `${formatUnits(usdc, 6)} USDC`;
  el.eurc.textContent = `${formatUnits(eurc, 6)} EURC`;
}
async function submitSettlement() {
  ...
  const hash3 = await wallet.writeContract({ chain: arc, account, address: token.address, abi: abi2, functionName: "transfer", args: [merchant, parseUnits(amount, token.decimals)] });
  await publicClient.waitForTransactionReceipt({ hash: hash3 });
  const rows = ledger();
  rows.unshift({ kind, token: symbol, amount, counterparty: merchant, reference, hash: hash3, at: (/* @__PURE__ */ new Date()).toISOString() });
  localStorage.setItem(activityKey, JSON.stringify(rows.slice(0, 20)));
  renderLedger();
  await refresh();
  setStatus(`${kind} confirmed.`, hash3);
}
