...
// circle/arc/src/arc-security-redeploy.ts
init_browser_buffer_global();
...
var SESSION_KEY = "arcSecurityRedeploy.v1";
var definitions = [
  {
...
var publicClient = createPublicClient({ chain: arcTestnet, transport: http(ARC_TESTNET.rpcUrls[0]) });
var records = restoreRecords();
...
var el = {
  ...
  deployNext: document.querySelector("#deployNext"),
  envOutput: document.querySelector("#envOutput"),
  exportManifest: document.querySelector("#exportManifest"),
  rows: document.querySelector("#deploymentRows"),
...
function explorerTransaction(hash3) {
...
function setStatus(message, hash3) {
  el.status.innerHTML = hash3 ? `${escapeHtml(message)} <a href="${explorerTransaction(hash3)}" target="_blank" rel="noreferrer">${hash3}</a>` : escapeHtml(message);
}
function render() {
  el.rows.innerHTML = records.map((record, index2) => {
    const oldAddress = record.oldAddress ? `<a href="${explorerAddress(record.oldAddress)}" target="_blank" rel="noreferrer">${short(record.oldAddress)}</a>` : "-";
    const deployment = record.deployment;
    const next2 = !deployment && records.findIndex((item) => !item.deployment) === index2;
    const status = deployment ? "deployed" : next2 ? "next" : "pending";
    const newAddress = deployment ? `<a href="${explorerAddress(deployment.address)}" target="_blank" rel="noreferrer">${short(deployment.address)}</a>` : "-";
    return `<tr><td>${index2 + 1}</td><td>${record.name}</td><td>${oldAddress}</td><td>${newAddress}</td><td><span class="state ${status}">${status}</span></td></tr>`;
  }).join("");
  el.envOutput.value = records.filter((record) => record.deployment).map((record) => `${record.envKey}=${record.deployment?.address}`).join("\n");
...
async function deployNext() {
  ...
  const index2 = records.findIndex((record2) => !record2.deployment);
  if (index2 < 0) return;
  ...
  try {
    ...
    setStatus(`${record.name} deployment submitted:`, hash3);
    ...
    records[index2] = { ...record, deployment: { address: receipt.contractAddress, transactionHash: hash3 } };
    localStorage.setItem(record.storageKey, receipt.contractAddress);
    persist();
    setStatus(`${record.name} deployed and runtime code verified:`, hash3);
  } catch (error) {
