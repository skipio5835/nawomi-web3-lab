...
// circle/arc/src/arc-installments.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcInstallmentPayments.contractAddress";
var DEFAULT_ACCOUNT = "0x0000000000000000000000000000000000000000";
var DEFAULT_MERCHANT = "0x78131700be4a8f2d16eeb0cba3498d2e717f2cd3";
var ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
var arcInstallmentsAbi = [
  {
    inputs: [
...
var el = {
  ...
  completeAgreement: document.querySelector("#completeAgreement"),
  ...
  merchantAddress: document.querySelector("#merchantAddress"),
  metadataURI: document.querySelector("#metadataURI"),
  nativeBalance: document.querySelector("#nativeBalance"),
  payInstallment: document.querySelector("#payInstallment"),
  payerAddress: document.querySelector("#payerAddress"),
  paymentURI: document.querySelector("#paymentURI"),
...
function agreementFromRaw(value) {
  ...
  return {
    ...
    agreementRef: value[8],
    metadataURI: value[9],
    completionURI: value[10]
  };
}
...
function updateActions() {
  ...
  const completeReady = open && (currentAgreement?.paidInstallments ?? 0) === (currentAgreement?.installmentCount ?? Number.NaN);
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcInstallmentPayments.json").then(
...
async function createAgreement() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [agreementId(), merchant, installmentCount, installmentAmount, label, metadataURI],
      account,
      chain: arcTestnet
    });
...
async function payInstallment() {
  ...
  try {
    ...
    setStatus("Payment submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshAgreement();
    setStatus("Installment paid:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function completeAgreement() {
  ...
  try {
    ...
    setStatus("Completion submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshAgreement();
    setStatus("Agreement completed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
