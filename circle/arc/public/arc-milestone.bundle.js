...
// circle/arc/src/arc-milestone.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcMilestone.contractAddress";
...
var el = {
  ...
  closeAgreement: document.querySelector("#closeAgreement"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createAgreement: document.querySelector("#createAgreement"),
  ...
  milestoneIndex: document.querySelector("#milestoneIndex"),
  milestoneStatus: document.querySelector("#milestoneStatus"),
  nativeBalance: document.querySelector("#nativeBalance"),
  refresh: document.querySelector("#refresh"),
  refundTo: document.querySelector("#refundTo"),
  releaseMilestone: document.querySelector("#releaseMilestone"),
  saveContract: document.querySelector("#saveContract"),
  secondAmount: document.querySelector("#secondAmount"),
  sponsorAddress: document.querySelector("#sponsorAddress"),
  statusLine: document.querySelector("#statusLine"),
  submissionURI: document.querySelector("#submissionURI"),
  submitMilestone: document.querySelector("#submitMilestone"),
  title: document.querySelector("#title"),
  walletAddress: document.querySelector("#walletAddress"),
  worker: document.querySelector("#worker"),
  workerAddress: document.querySelector("#workerAddress")
};
...
function agreementFromRaw(value) {
  const tuple = value;
  const rawAgreement = tuple[0];
  const milestoneCount = tuple[1] ?? 0n;
  const agreement = Array.isArray(rawAgreement) ? {
    sponsor: rawAgreement[0],
    worker: rawAgreement[1],
    totalFunded: rawAgreement[2],
    totalReleased: rawAgreement[3],
    createdAt: rawAgreement[4],
    closed: Boolean(rawAgreement[5]),
    title: rawAgreement[6],
    metadataURI: rawAgreement[7]
  } : rawAgreement;
  return {
...
function milestoneFromRaw(value) {
  if (!Array.isArray(value)) {
  ...
  return {
    ...
    submissionURI: value[3]
  };
}
function updateAgreementIdDisplay() {
  try {
...
function renderAgreement() {
  ...
  el.milestoneStatus.textContent = currentMilestone ? `${formatEther(currentMilestone.amount)} USDC, ${currentMilestone.submitted ? "submitted" : "not submitted"}, ${currentMilestone.released ? "released" : "not released"}` : "-";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcMilestoneAgreement.json").then(
...
async function createAgreement() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [agreementId(), worker, [firstAmount, secondAmount], title, metadataURI],
      account,
      chain: arcTestnet,
      value: total
    });
    ...
    setStatus("Agreement created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function submitMilestone() {
  ...
  try {
    ...
    setStatus("Submit submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshAgreement();
    setStatus("Milestone submitted:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function releaseMilestone() {
  ...
  try {
    ...
    setStatus("Release submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshAgreement();
    setStatus("Milestone released:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeAgreement() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshAgreement();
    setStatus("Agreement closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
