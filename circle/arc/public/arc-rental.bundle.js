...
var CONTRACT_KEY = "ArcRentalEscrow.contractAddress";
...
var el = {
  bookRental: document.querySelector("#bookRental"),
  cancelRental: document.querySelector("#cancelRental"),
  cancelURI: document.querySelector("#cancelURI"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createRental: document.querySelector("#createRental"),
  ...
  payoutTo: document.querySelector("#payoutTo"),
  refresh: document.querySelector("#refresh"),
  refundTo: document.querySelector("#refundTo"),
  rentalFee: document.querySelector("#rentalFee"),
  rentalId: document.querySelector("#rentalId"),
  rentalStatus: document.querySelector("#rentalStatus"),
  renterAddress: document.querySelector("#renterAddress"),
  returnRental: document.querySelector("#returnRental"),
  returnURI: document.querySelector("#returnURI"),
...
function updateRentalIdDisplay() {
  try {
...
function renderRental() {
  ...
  const state = currentRental.closed ? currentRental.canceled ? "canceled" : "returned" : currentRental.renter === ZERO_ADDRESS ? "listed" : "booked";
  ...
  el.renterAddress.innerHTML = currentRental.renter === ZERO_ADDRESS ? "-" : `<a href="${addressUrl(currentRental.renter)}" target="_blank" rel="noreferrer">${shortValue(
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcRentalEscrow.json").then(
...
async function createRental() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [rentalId(), payoutTo, rentalFee, deposit, title, metadataURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshRental();
    setStatus("Rental created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function bookRental() {
  ...
  try {
    ...
    setStatus("Book submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshRental();
    setStatus("Rental booked:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function returnRental() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [rentalId(), damageFee, refundTo, returnURI],
      account,
      chain: arcTestnet
    });
    setStatus("Return submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshRental();
    setStatus("Rental returned:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function cancelRental() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [rentalId(), refundTo, cancelURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshRental();
    setStatus("Rental canceled:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
