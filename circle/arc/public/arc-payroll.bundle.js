...
var CONTRACT_KEY = "arcPayrollVault.contractAddress";
...
var el = {
  ...
  claimPayroll: document.querySelector("#claimPayroll"),
  claimTo: document.querySelector("#claimTo"),
  claimURI: document.querySelector("#claimURI"),
  closePayroll: document.querySelector("#closePayroll"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createPayroll: document.querySelector("#createPayroll"),
  ...
  payrollId: document.querySelector("#payrollId"),
...
function updatePayrollIdDisplay() {
  try {
...
function renderPayroll() {
  ...
  el.payrollStatus.textContent = currentPayroll.closed ? "closed" : currentPayroll.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcPayrollVault.json").then((response) => response.json());
...
async function createPayroll() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [payrollId(), worker, 0n, reference, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Payroll created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimPayroll() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [payrollId(), claimTo, claimURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshPayroll();
    setStatus("Payroll claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closePayroll() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshPayroll();
    setStatus("Payroll closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
