...
// circle/arc/src/arc-support-desk.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "ArcSupportDesk.contractAddress";
...
var el = {
  ...
  closeURI: document.querySelector("#closeURI"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createTicket: document.querySelector("#createTicket"),
  ...
  requestAgent: document.querySelector("#requestAgent"),
  requestOwner: document.querySelector("#requestOwner"),
  respondTicket: document.querySelector("#respondTicket"),
  responseURI: document.querySelector("#responseURI"),
  saveContract: document.querySelector("#saveContract"),
  statusLine: document.querySelector("#statusLine"),
  ticketId: document.querySelector("#ticketId"),
  ticketRef: document.querySelector("#ticketRef"),
...
function renderTicket() {
  ...
  el.ticketStatus.textContent = currentTicket.closed ? "closed" : currentTicket.responded ? "responded" : "open";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcSupportDesk.json").then(
...
async function createTicket() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [ticketId(), agent, ticketRef, category, metadataURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshTicket();
    setStatus("Support ticket created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function respondTicket() {
  ...
  try {
    ...
    setStatus("Response submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshTicket();
    setStatus("Support ticket responded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeTicket() {
  ...
  try {
    ...
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshTicket();
    setStatus("Support ticket closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
