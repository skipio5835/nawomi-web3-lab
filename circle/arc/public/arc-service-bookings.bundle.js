...
// circle/arc/src/arc-service-bookings.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcServiceBookings.contractAddress";
var DEFAULT_ACCOUNT = "0x0000000000000000000000000000000000000000";
var ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
var arcServiceBookingsAbi = [
  {
    inputs: [
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcServiceBookings.json").then(
...
async function createService() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [serviceId(), treasury, price, maxBookings, title, metadataURI],
      account,
      chain: arcTestnet
    });
...
async function buyBooking() {
  ...
  try {
    ...
    setStatus("Booking submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshService();
    setStatus("Service booked:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function completeBooking() {
  ...
  try {
    ...
    setStatus("Complete submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshService();
    setStatus("Booking completed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function refundBooking() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [serviceId(), currentBookingId, refundTo],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshService();
    setStatus("Booking refunded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function settleService() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshService();
    setStatus("Service settled:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
