...
// circle/arc/src/arc-cashback.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "ArcCashbackVault.contractAddress";
...
var currentCashback = null;
var el = {
  amount: document.querySelector("#amount"),
  CashbackId: document.querySelector("#CashbackId"),
  CashbackStatus: document.querySelector("#CashbackStatus"),
  claimValue: document.querySelector("#claimValue"),
  closeCashback: document.querySelector("#closeCashback"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createCashback: document.querySelector("#createCashback"),
  creatorAddress: document.querySelector("#creatorAddress"),
  ...
  claimCashback: document.querySelector("#claimCashback"),
  claimTo: document.querySelector("#claimTo"),
...
function CashbackFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function updateCashbackIdDisplay() {
  try {
...
function renderCashback() {
  ...
  el.CashbackStatus.textContent = currentCashback.closed ? "closed" : currentCashback.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcCashbackVault.json").then(
...
async function createCashback() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [CashbackId(), recipient, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Cashback created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimCashback() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [CashbackId(), claimTo, claimURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshCashback();
    setStatus("Cashback claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeCashback() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [CashbackId(), refundTo],
      account,
      chain: arcTestnet
    });
    setStatus("Close submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshCashback();
    setStatus("Cashback closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
