...
var CONTRACT_KEY = "arcExpense.contractAddress";
...
var currentContribution = 0n;
var el = {
  closeExpense: document.querySelector("#closeExpense"),
  ...
  createExpense: document.querySelector("#createExpense"),
  ...
  payeeAddress: document.querySelector("#payeeAddress"),
...
function expenseFromRaw(value) {
  ...
  return {
    owner: value[0],
    payee: value[1],
    targetAmount: value[2],
    totalContributed: value[3],
    totalWithdrawn: value[4],
    createdAt: value[5],
...
function updateExpenseIdDisplay() {
  try {
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcExpenseSplitter.json").then(
...
async function createExpense() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [expenseId(), payee, targetAmount, title, metadataURI],
      account,
      chain: arcTestnet
    });
    ...
    setStatus("Expense created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function contribute() {
  ...
  try {
    ...
    setStatus("Contribution submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshExpense();
    setStatus("Contribution recorded:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function withdraw() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [expenseId(), amount, withdrawTo],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshExpense();
    setStatus("Expense withdrawn:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeExpense() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshExpense();
    setStatus("Expense closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
