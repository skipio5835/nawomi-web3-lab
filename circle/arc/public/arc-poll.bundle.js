...
var CONTRACT_KEY = "arcPoll.contractAddress";
...
var el = {
  ...
  closePoll: document.querySelector("#closePoll"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createPoll: document.querySelector("#createPoll"),
  ...
  pollCreator: document.querySelector("#pollCreator"),
  pollId: document.querySelector("#pollId"),
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcPoll.json").then((response) => response.json());
...
async function castVote() {
  ...
  try {
    ...
    setStatus("Vote submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshPoll();
    setStatus("Vote cast:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
