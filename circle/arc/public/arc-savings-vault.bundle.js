...
// circle/arc/src/arc-savings-vault.ts
init_browser_buffer_global();
...
var el = {
  closeVault: document.querySelector("#closeVault"),
  ...
  setGoal: document.querySelector("#setGoal"),
  statusLine: document.querySelector("#statusLine"),
  vaultBalance: document.querySelector("#vaultBalance"),
  vaultId: document.querySelector("#vaultId"),
  vaultLabel: document.querySelector("#vaultLabel"),
...
el.vaultLabel.value = `arc-vault-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`;
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcSavingsVault.json").then((response) => response.json());
...
async function deposit() {
  ...
  try {
    ...
    setStatus("Deposit submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshVault();
    setStatus("Deposit confirmed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function setGoal() {
  ...
  try {
    ...
    setStatus("Goal update submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshVault();
    setStatus("Goal updated:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function withdraw() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshVault();
    setStatus("Withdraw confirmed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
