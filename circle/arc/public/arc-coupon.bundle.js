...
var CONTRACT_KEY = "ArcCouponVault.contractAddress";
...
var el = {
  ...
  claimCoupon: document.querySelector("#claimCoupon"),
...
function updateCouponIdDisplay() {
  try {
...
function renderCoupon() {
  ...
  el.CouponStatus.textContent = currentCoupon.closed ? "closed" : currentCoupon.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcCouponVault.json").then(
...
async function createCoupon() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [couponId(), recipient, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    await refreshBalance();
    await refreshCoupon();
    setStatus("Coupon created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimCoupon() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [couponId(), claimTo, claimURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshCoupon();
    setStatus("Coupon claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
