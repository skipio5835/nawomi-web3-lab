...
var CONTRACT_KEY = "arcBounty.contractAddress";
var DEFAULT_SPONSOR = "0x0000000000000000000000000000000000000000";
var ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";
var STATUS_LABELS = ["none", "open", "accepted", "submitted", "released", "canceled"];
var arcBountyAbi = [
  {
    inputs: [
...
var currentBounty = null;
var el = {
  acceptBounty: document.querySelector("#acceptBounty"),
  bountyId: document.querySelector("#bountyId"),
  bountyStatus: document.querySelector("#bountyStatus"),
  cancelBounty: document.querySelector("#cancelBounty"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createBounty: document.querySelector("#createBounty"),
  ...
  releaseBounty: document.querySelector("#releaseBounty"),
  ...
  submitWork: document.querySelector("#submitWork"),
...
function bountyFromRaw(value) {
  ...
  return {
    ...
    title: value[8],
    metadataURI: value[9],
    submissionURI: value[10]
  };
}
function updateBountyIdDisplay() {
  try {
...
function renderBounty() {
  ...
  el.workerAddress.innerHTML = currentBounty.worker === ZERO_ADDRESS ? "-" : `<a href="${addressUrl(currentBounty.worker)}" target="_blank" rel="noreferrer">${shortValue(
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcBountyBoard.json").then(
...
async function createBounty() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshBounty();
    setStatus("Bounty created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function acceptBounty() {
  ...
  try {
    ...
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBounty();
    setStatus("Bounty accepted:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function submitWork() {
  ...
  try {
    ...
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBounty();
    setStatus("Work submitted:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function releaseBounty() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshBounty();
    setStatus("Bounty released:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function cancelBounty() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshBounty();
    setStatus("Bounty canceled:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
