...
// circle/arc/src/arc-gift-card.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcGiftCardVault.contractAddress";
...
var el = {
  ...
  closeGiftCard: document.querySelector("#closeGiftCard"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createGiftCard: document.querySelector("#createGiftCard"),
  ...
  redeemGiftCard: document.querySelector("#redeemGiftCard"),
  redeemTo: document.querySelector("#redeemTo"),
  redeemURI: document.querySelector("#redeemURI"),
...
function renderCard() {
  ...
  el.cardStatus.textContent = currentCard.closed ? "closed" : currentCard.redeemedAmount > 0n ? "redeemed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcGiftCardVault.json").then(
...
async function createGiftCard() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [cardId(), recipient, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Gift card created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function redeemGiftCard() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [cardId(), redeemTo, redeemURI],
      account,
      chain: arcTestnet
    });
    setStatus("Redeem submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshGiftCard();
    setStatus("Gift card redeemed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeGiftCard() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshGiftCard();
    setStatus("Gift card closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
