...
var CONTRACT_KEY = "arcVestingVault.contractAddress";
...
var el = {
  ...
  claimVesting: document.querySelector("#claimVesting"),
  closeVesting: document.querySelector("#closeVesting"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createVesting: document.querySelector("#createVesting"),
  deployContract: document.querySelector("#deployContract"),
  funderAddress: document.querySelector("#funderAddress"),
  grantId: document.querySelector("#grantId"),
  ...
  vestingStatus: document.querySelector("#vestingStatus"),
...
function renderGrant() {
  ...
  el.vestingStatus.textContent = currentGrant.closed ? "closed" : currentGrant.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcVestingVault.json").then((response) => response.json());
...
async function createVesting() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [grantId(), beneficiary, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Vesting grant created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimVesting() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshVesting();
    setStatus("Vesting claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeVesting() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshVesting();
    setStatus("Vesting closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
