...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account2 ? encode4(from11("constructor(bytes, bytes)"), {
    bytecode: deploylessCallViaBytecodeBytecode,
    args: [
      getBalanceCode,
...
var CONTRACT_KEY = "arcAirdrop.hardenedContractAddress.v2";
var DEFAULT_CONTRACT_ADDRESS = "0x5E67aB874F36897f559F141D47Fc0d4bCDe70209";
var DEFAULT_RECIPIENT = "0x0000000000000000000000000000000000000000";
...
var el = {
  allocationAmount: document.querySelector("#allocationAmount"),
  campaignBalance: document.querySelector("#campaignBalance"),
  ...
  closeCampaign: document.querySelector("#closeCampaign"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createCampaign: document.querySelector("#createCampaign"),
...
function campaignId() {
  ...
  const owner = account ?? DEFAULT_RECIPIENT;
  return keccak256(encodePacked(["address", "string"], [owner, label]));
}
function campaignFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function allocationFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function updateCampaignIdDisplay() {
  try {
...
function renderCampaign() {
  ...
  const expired = BigInt(Math.floor(Date.now() / 1e3)) > currentCampaign.closesAt;
  el.campaignBalance.textContent = `${formatEther(currentCampaign.totalClaimed)} claimed / ${formatEther(
    remaining
  )} remaining`;
  el.campaignStatus.textContent = currentCampaign.closed ? "closed" : expired ? "expired" : claimStatus;
...
function updateActions() {
  ...
  const expired = Boolean(
    currentCampaign && BigInt(Math.floor(Date.now() / 1e3)) > currentCampaign.closesAt
  );
  const isOwner = Boolean(
    account && currentCampaign && account.toLowerCase() === currentCampaign.owner.toLowerCase()
  );
  const claimable = !expired && (currentAllocation?.claimable ?? 0n) > 0n && !currentAllocation?.claimed;
  el.claim.disabled = !hasWallet || !hasContract || !openCampaign || !claimable;
  el.closeCampaign.disabled = !hasWallet || !hasContract || !openCampaign || !expired || !isOwner;
...
async function getEthereumProvider() {
  ...
  const metaMask = announced.find((item) => item.rdns?.toLowerCase() === "io.metamask") ?? announced.find((item) => item.name?.trim().toLowerCase() === "metamask");
  if (metaMask) return metaMask.provider;
  const injected = window.ethereum;
  const legacyMetaMask = injected?.providers?.find((provider) => provider.isMetaMask && !provider.isBinance && !provider.isBinanceChain);
  if (legacyMetaMask) return legacyMetaMask;
  if (injected?.isMetaMask && !injected.isBinance && !injected.isBinanceChain) return injected;
  throw new Error("MetaMask provider not found. Enable MetaMask and reload this page.");
}
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcAirdropCampaign.json").then(
...
async function createCampaign() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [campaignId(), [recipient], [amount], BigInt(durationMinutes * 60), label],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Campaign created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claim() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshCampaign();
    setStatus("Airdrop claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeCampaign() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshCampaign();
    setStatus("Campaign closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
