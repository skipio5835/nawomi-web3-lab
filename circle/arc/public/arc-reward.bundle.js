...
var CONTRACT_KEY = "ArcRewardVault.contractAddress";
...
var el = {
  ...
  closeReward: document.querySelector("#closeReward"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createReward: document.querySelector("#createReward"),
...
function updateRewardIdDisplay() {
  try {
...
function renderReward() {
  ...
  el.rewardStatus.textContent = currentReward.closed ? "closed" : currentReward.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcRewardVault.json").then(
...
async function createReward() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [rewardId(), recipient, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    await refreshBalance();
    await refreshReward();
    setStatus("Reward created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimReward() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [rewardId(), claimTo, claimURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshReward();
    setStatus("Reward claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
