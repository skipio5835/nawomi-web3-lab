...
// circle/arc/src/arc-warranty.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "ArcWarrantyRegistry.contractAddress";
...
var el = {
  claimURI: document.querySelector("#claimURI"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  deployContract: document.querySelector("#deployContract"),
  expiresDays: document.querySelector("#expiresDays"),
  metadataURI: document.querySelector("#metadataURI"),
  nativeBalance: document.querySelector("#nativeBalance"),
  openClaim: document.querySelector("#openClaim"),
  ownerAddress: document.querySelector("#ownerAddress"),
  productRef: document.querySelector("#productRef"),
  refresh: document.querySelector("#refresh"),
  registerWarranty: document.querySelector("#registerWarranty"),
  resolutionURI: document.querySelector("#resolutionURI"),
  resolveClaim: document.querySelector("#resolveClaim"),
  saveContract: document.querySelector("#saveContract"),
  serviceProvider: document.querySelector("#serviceProvider"),
  serviceProviderView: document.querySelector("#serviceProviderView"),
  statusLine: document.querySelector("#statusLine"),
  title: document.querySelector("#title"),
  walletAddress: document.querySelector("#walletAddress"),
  warrantyId: document.querySelector("#warrantyId"),
  warrantyStatus: document.querySelector("#warrantyStatus")
};
...
function expiresAt() {
  ...
  if (days === 0) return 0n;
  return BigInt(Math.floor(Date.now() / 1e3) + Math.floor(days * 86400));
}
function warrantyFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
  ...
  return {
    ...
    metadataURI: value[9],
    claimURI: value[10],
    resolutionURI: value[11]
  };
}
function updateWarrantyIdDisplay() {
  try {
...
function renderWarranty() {
  ...
  el.warrantyStatus.textContent = currentWarranty.resolved ? "resolved" : currentWarranty.claimOpen ? "claim open" : "registered";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcWarrantyRegistry.json").then(
...
async function refreshWarranty() {
...
async function registerWarranty() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [warrantyId(), serviceProvider, expiresAt(), productRef, metadataURI],
      account,
      chain: arcTestnet
    });
    setStatus("Register submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshWarranty();
    setStatus("Warranty registered:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function openClaim() {
  ...
  try {
    ...
    setStatus("Claim submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshWarranty();
    setStatus("Warranty claim opened:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function resolveClaim() {
  ...
  try {
    ...
    setStatus("Resolve submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshWarranty();
    setStatus("Warranty claim resolved:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
