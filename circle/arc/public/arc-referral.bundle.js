...
// circle/arc/src/arc-referral.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "ArcReferralVault.contractAddress";
...
var currentReferral = null;
var el = {
  amount: document.querySelector("#amount"),
  ReferralId: document.querySelector("#ReferralId"),
  ReferralStatus: document.querySelector("#ReferralStatus"),
  claimValue: document.querySelector("#claimValue"),
  closeReferral: document.querySelector("#closeReferral"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  createReferral: document.querySelector("#createReferral"),
  ...
  claimReferral: document.querySelector("#claimReferral"),
...
function ReferralFromRaw(value) {
  if (!Array.isArray(value)) {
    const object = value;
    return {
...
function updateReferralIdDisplay() {
  try {
...
function renderReferral() {
  ...
  el.ReferralStatus.textContent = currentReferral.closed ? "closed" : currentReferral.claimedAmount > 0n ? "claimed" : "funded";
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcReferralVault.json").then(
...
async function createReferral() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [ReferralId(), recipient, 0n, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount
    });
    ...
    setStatus("Referral created:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function claimReferral() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [ReferralId(), claimTo, claimURI],
      account,
      chain: arcTestnet
    });
    ...
    await refreshBalance();
    await refreshReferral();
    setStatus("Referral claimed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function closeReferral() {
  ...
  try {
    ...
    await refreshBalance();
    await refreshReferral();
    setStatus("Referral closed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
