...
var el = {
  ...
  native: document.querySelector("#nativeBalance"),
  usdc: document.querySelector("#usdcBalance"),
  eurc: document.querySelector("#eurcBalance"),
  usyc: document.querySelector("#usycBalance"),
...
async function refreshBalances() {
  ...
  const [native, ...balances] = await Promise.all([
    ...
    ...Object.values(tokens).map((token) => publicClient.readContract({ address: token.address, abi: erc20Abi2, functionName: "balanceOf", args: [account] }))
  ]);
...
async function sendToken() {
  ...
  renderActivity();
  await refreshBalances();
  setStatus(`${symbol} transfer confirmed.`, hash3);
}
