...
var CONTRACT_KEY = "ArcAccessRegistry.contractAddress";
...
var el = {
  accessId: document.querySelector("#accessId"),
  accessRef: document.querySelector("#accessRef"),
  accessStatus: document.querySelector("#accessStatus"),
  approvalURI: document.querySelector("#approvalURI"),
  approveAccess: document.querySelector("#approveAccess"),
  approver: document.querySelector("#approver"),
  approverView: document.querySelector("#approverView"),
  ...
  requestAccess: document.querySelector("#requestAccess"),
  requesterView: document.querySelector("#requesterView"),
  revokeAccess: document.querySelector("#revokeAccess"),
  revokeURI: document.querySelector("#revokeURI"),
...
function accessFromRaw(value) {
  ...
  return {
    ...
    role: value[8],
    metadataURI: value[9],
    approvalURI: value[10],
    revokeURI: value[11]
  };
}
function updateAccessIdDisplay() {
  try {
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcAccessRegistry.json").then(
...
async function requestAccess() {
  ...
  try {
    ...
    const hash3 = await walletClient.writeContract({
      ...
      args: [accessId(), approver, accessRef, role, metadataURI],
      account,
      chain: arcTestnet
    });
    setStatus("Request submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshAccess();
    setStatus("Access requested:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function approveAccess() {
  ...
  try {
    ...
    setStatus("Approval submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshAccess();
    setStatus("Access approved:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function revokeAccess() {
  ...
  try {
    ...
    setStatus("Revoke submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshAccess();
    setStatus("Access revoked:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
