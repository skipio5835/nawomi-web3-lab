...
// circle/arc/src/arc-membership.ts
init_browser_buffer_global();
...
var CONTRACT_KEY = "arcMembership.contractAddress";
var DAY_SECONDS = 86400;
var arcMembershipAbi = [
  {
...
var el = {
  approvePass: document.querySelector("#approvePass"),
  approvedAddress: document.querySelector("#approvedAddress"),
  connect: document.querySelector("#connect"),
  contractAddress: document.querySelector("#contractAddress"),
  deployContract: document.querySelector("#deployContract"),
  durationDays: document.querySelector("#durationDays"),
  expiresAt: document.querySelector("#expiresAt"),
  handle: document.querySelector("#handle"),
  mintPass: document.querySelector("#mintPass"),
  mintPrice: document.querySelector("#mintPrice"),
  nativeBalance: document.querySelector("#nativeBalance"),
  ownerAddress: document.querySelector("#ownerAddress"),
  refresh: document.querySelector("#refresh"),
  renewalDays: document.querySelector("#renewalDays"),
  renewalPrice: document.querySelector("#renewalPrice"),
  renewPass: document.querySelector("#renewPass"),
  revokeApproval: document.querySelector("#revokeApproval"),
  saveContract: document.querySelector("#saveContract"),
  spender: document.querySelector("#spender"),
  statusLine: document.querySelector("#statusLine"),
  tokenHandle: document.querySelector("#tokenHandle"),
  tokenId: document.querySelector("#tokenId"),
  walletAddress: document.querySelector("#walletAddress")
};
el.contractAddress.value = contractAddress;
el.durationDays.value = "30";
el.handle.value = `arc-member-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`;
...
async function deployContract2() {
  ...
  try {
    ...
    const artifact = await fetch("/public/artifacts/ArcMembershipPass.json").then((response) => response.json());
...
async function mintPass() {
  ...
  try {
    ...
    setStatus("Mint submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshContractState();
    setStatus("Membership minted:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function renewPass() {
  ...
  try {
    ...
    setStatus("Renew submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshBalance();
    await refreshContractState();
    setStatus("Membership renewed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function approvePass() {
  ...
  try {
    ...
    setStatus("Approve submitted:", hash3);
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshContractState();
    setStatus("NFT approval confirmed:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
...
async function revokeApproval() {
  ...
  try {
    ...
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    await refreshContractState();
    setStatus("NFT approval revoked:", hash3);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
