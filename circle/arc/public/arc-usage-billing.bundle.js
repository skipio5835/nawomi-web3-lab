...
var storageKey = "ArcUsageBilling.contractAddress";
var provider = null;
var wallet = null;
var account = null;
var artifact = null;
...
el.chargeId.value = `arc-usage-${date}`;
el.units.value = "10";
el.unitPrice.value = "0.001";
...
async function getArtifact() {
  artifact ??= await fetch("./artifacts/ArcUsageBilling.json").then((response) => response.json());
  return artifact;
}
...
function chargeHash() {
  const value = el.chargeId.value.trim();
  if (!value) throw new Error("Charge ID is required.");
  return keccak256(toBytes(value));
}
function expirySeconds() {
  const value = BigInt(Math.floor(new Date(el.expiry.value).getTime() / 1e3));
  if (value <= BigInt(Math.floor(Date.now() / 1e3))) throw new Error("Expiry must be in the future.");
  return value;
}
async function deploy() {
  await ensureWallet();
  const compiled = await getArtifact();
  setStatus("Waiting for MetaMask: deploy ArcUsageBilling...");
...
async function createCharge() {
  ...
  const units = BigInt(el.units.value);
  const unitPrice = parseEther(el.unitPrice.value);
  setStatus("Waiting for MetaMask: create usage charge...");
  const hash3 = await wallet.writeContract({ address: requireContract(), abi: compiled.abi, functionName: "createCharge", args: [chargeHash(), el.payer.value.trim(), units, unitPrice, expirySeconds(), el.metadata.value.trim()] });
  await publicClient.waitForTransactionReceipt({ hash: hash3 });
  setStatus("Usage charge created.", hash3);
  await loadCharge();
}
async function loadCharge() {
  const compiled = await getArtifact();
  const charge = await publicClient.readContract({ address: requireContract(), abi: compiled.abi, functionName: "getCharge", args: [chargeHash()] });
  const status = ["unknown", "open", "paid", "cancelled"];
  el.result.textContent = JSON.stringify({ chargeId: chargeHash(), merchant: charge.merchant ?? charge[0], payer: charge.payer ?? charge[1], units: String(charge.units ?? charge[2]), unitPrice: `${formatEther(charge.unitPrice ?? charge[3])} USDC`, totalAmount: `${formatEther(charge.totalAmount ?? charge[4])} USDC`, expiresAt: new Date(Number(charge.expiresAt ?? charge[6]) * 1e3).toISOString(), status: status[Number(charge.status ?? charge[9])] ?? "unknown", metadataURI: charge.metadataURI ?? charge[8] }, null, 2);
  const state = Number(charge.status ?? charge[9]);
  el.pay.disabled = state !== 1;
  el.cancel.disabled = state !== 1;
  setStatus("Usage charge loaded.");
}
async function payCharge() {
  await ensureWallet();
  const compiled = await getArtifact();
  const units = BigInt(el.units.value);
  const total = units * parseEther(el.unitPrice.value);
  setStatus("Waiting for MetaMask: pay usage charge...");
  const hash3 = await wallet.writeContract({ address: requireContract(), abi: compiled.abi, functionName: "payCharge", args: [chargeHash()], value: total });
  await publicClient.waitForTransactionReceipt({ hash: hash3 });
  setStatus("Usage charge paid and settled to merchant.", hash3);
  await loadCharge();
}
async function cancelCharge() {
  await ensureWallet();
  const compiled = await getArtifact();
  const hash3 = await wallet.writeContract({ address: requireContract(), abi: compiled.abi, functionName: "cancelCharge", args: [chargeHash()] });
  await publicClient.waitForTransactionReceipt({ hash: hash3 });
...
el.load.addEventListener("click", () => void loadCharge().catch((error) => setStatus(error instanceof Error ? error.message : "Load failed.")));
el.pay.addEventListener("click", () => void payCharge().catch((error) => setStatus(error instanceof Error ? error.message : "Payment failed.")));
el.cancel.addEventListener("click", () => void cancelCharge().catch((error) => setStatus(error instanceof Error ? error.message : "Cancel failed.")));
