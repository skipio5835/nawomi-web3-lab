...
const el = {
  ...
  deadlineHours: document.querySelector<HTMLInputElement>("#deadlineHours")!,
  deadlineStatus: document.querySelector<HTMLElement>("#deadlineStatus")!,
...
function renderDeposit(): void {
  ...
  el.deadlineStatus.textContent = new Date(Number(currentDeposit.deadline) * 1000).toLocaleString();
...
async function createDeposit(): Promise<void> {
  ...
  try {
    ...
    const deadline = BigInt(Math.floor(Date.now() / 1000) + Math.round(deadlineHours * 60 * 60));
    ...
    const hash = await walletClient.writeContract({
      ...
      args: [depositId(), beneficiary as Address, deadline, label, metadataURI],
      account,
      chain: arcTestnet,
      value: amount,
    });
