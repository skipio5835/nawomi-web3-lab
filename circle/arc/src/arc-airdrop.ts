...
const CONTRACT_KEY = "arcAirdrop.hardenedContractAddress.v2";
const DEFAULT_CONTRACT_ADDRESS = "0x5E67aB874F36897f559F141D47Fc0d4bCDe70209" as Address;
...
function renderCampaign(): void {
  ...
  const expired = BigInt(Math.floor(Date.now() / 1000)) > currentCampaign.closesAt;
...
function updateActions(): void {
  ...
  const expired = Boolean(
    currentCampaign && BigInt(Math.floor(Date.now() / 1000)) > currentCampaign.closesAt,
  );
...
async function getEthereumProvider(): Promise<EIP1193Provider> {
  ...
  const metaMask = announced.find((item) => item.rdns?.toLowerCase() === "io.metamask")
    ?? announced.find((item) => item.name?.trim().toLowerCase() === "metamask");
  if (metaMask) return metaMask.provider;

  const injected = window.ethereum as InjectedProvider | undefined;
  const legacyMetaMask = injected?.providers?.find((provider) => (
    provider.isMetaMask && !provider.isBinance && !provider.isBinanceChain
  ));
