...
const el = {
  ...
  pendingRefund: document.querySelector<HTMLElement>("#pendingRefund")!,
  ...
  withdrawRefund: document.querySelector<HTMLButtonElement>("#withdrawRefund")!,
};
...
function renderPendingRefund(): void {
...
async function refreshPendingRefund(): Promise<void> {
...
async function withdrawRefund(): Promise<void> {
  if (!walletClient || !account) await connect();
  if (!walletClient || !account || !contractAddress) return;
  ...
  try {
    ...
    const hash = await walletClient.writeContract({
      ...
      args: [refundTo as Address],
      account,
      chain: arcTestnet,
    });
    setStatus("Refund withdrawal submitted:", hash);
    await waitForSuccess(hash, "Withdraw bid refund");
    await refreshBalance();
    await refreshPendingRefund();
    setStatus("Bid refund withdrawn:", hash);
  } catch (error) {
    setStatus(errorMessage(error));
  } finally {
    updateActions();
